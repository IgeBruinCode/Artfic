...
Rauwe "operationele blauwdruk": de pagina leest als een technisch spec-document dat op groot formaat is afgedrukt. Uitgesproken typografische schaal (H1 tot 9rem, alles kapitaal), harde 4px-lijnen, full-bleed donkere navy zones afgewisseld met wit, zichtbare sectiecodes (`00 / INTRO` … `07 / CONTACT`), gele signaalstroken voor kernclaims en oversized moduleplaten met harde offset-schaduwen in effen merkblauw. Geen foto's, illustraties, iconengrids, afronding, gradients of transparantie; het enige beeldmateriaal is het officiële witte Artific-logo op de donkere commandobar en footer. De compositie (donkere sticky commandobar met scrollvoortgang, verspringende platen, verticale pipeline, genummerde vakken) verschilt bewust van de rustige editorial leeskolom van `/minimalistisch/`.
...
| Artific-navy | `#042244` | full-bleed donkere blokken en commandobar; tekst en 4px-lijnen op licht; gevulde koplabels, primaire CTA en footer (één CSS-token `--navy`) | ± 42% |
...
| Artific-geel | `#FFD602` | signaal: accentranden van de donkere blokken, signaalstroken, slot-CTA, security-kopjes en focusring op donker | ± 1–2% |

Contrastregels: lopende tekst is altijd `#042244` op wit/lichtblauw of `#FFFFFF` op navy (ruim boven WCAG AA 4,5:1). `#287CEB` haalt tegen wit/navy slechts ± 3,9–4,1:1 en wordt daarom **nooit** met kleine tekst gecombineerd: het is beperkt tot vlakken zonder tekst (voortgangsbalk, offset-schaduwen), grote vette decoratieve cijfers (≥ 24px/700, WCAG-grens 3:1) en de 3px focusring op licht (non-text, ≥ 3:1). Sectiecodes staan in `#042244` op licht en `#FFD602` op donker; het Artific-pipelinestation is `#042244` met gele rand en witte tekst; de bar-CTA is geel vlak met navy tekst (± 11,3:1). `#FFD602` staat als tekstkleur alleen op `#042244` (± 11,3:1) en als vlak altijd met `#042244`-tekst.
...
- Zichtbare structuur: elk blok opent met een omkaderde sectiecode (`aria-hidden`; navy op licht, geel op donker), fasen en stappen tonen grote `decimal-leading-zero`-tellers in merkblauw, moduleplaten dragen oversized nummers (tot 7rem).
- Verspringing op ≥1000px: hero in 7/4-grid, moduleplaten 02 en 03 schuiven horizontaal in; op mobiel valt alles lineair terug.

## Componentstijl

- **Commandobar:** sticky, navy met 4px gele onderrand en een 6px blauwe scroll-voortgangsbalk (decoratief, `aria-hidden`); wit logo, kapitaal-navigatie (verborgen ≤480px), gele bar-CTA met navy tekst. Alle interactieve doelen ≥ 44px.
- **CTA's:** blokvormig, 4px rand, kapitaal, gewicht 800, ≥ 52px hoog. Primair: navy vlak + 8px blauwe offset-schaduw; secundair: wit omlijnd; slot: geel vlak respectievelijk wit omlijnd op navy. Hover: 3px onderstreping (CSS) plus −3px GSAP-offset; focus: 3px ring in blauw (licht) of geel (donker).
- **Signaalstroken:** gele balken met 4px rand voor kernclaims (award, "Artific lost ze op", ChatGPT≠GPT, governance-balans).
- **Vragen/fasen/specsheet:** aaneengesloten omkaderde rijen met 4px scheidingslijnen; vraagkoppen als gevulde navy labels; security-specsheet op ≥700px twee kolommen met doorlopende witte lijnen.
- **Pipeline:** drie omkaderde stations (Artific-station navy gevuld met gele rand) verticaal met ↓-koppelingen; op ≥1000px horizontaal met →. Koppelingen `aria-hidden`, het geheel heeft een beschrijvend `aria-label`.
...
- **Footer:** navy met gele toprand, wit logo, officiële contactlinks (contactpagina, e-mail, telefoon).
