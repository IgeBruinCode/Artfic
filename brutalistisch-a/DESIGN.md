...
| Artific-blauw | `#287CEB` | sectiecodes, faseringnummers, offset-schaduwen van platen en CTA's, Artific-pipelinestation, bar-CTA, focusring op licht | ± 8% |
...
Contrastregels: lopende tekst is altijd `#0A213D` op wit/lichtblauw of `#FFFFFF` op marine-/donkerblauw (ruim boven WCAG AA). `#287CEB` wordt op wit alleen voor grote vette cijfers/codes en vlakken gebruikt, nooit voor kleine informatieve tekst. `#ECA414` staat als tekstkleur alleen op `#062244`/`#0A213D` (ruim AA) en als vlak altijd met `#0A213D`-tekst.
...
- Zichtbare structuur: elk blok opent met een omkaderde sectiecode (`aria-hidden`), fasen en stappen tonen `decimal-leading-zero`-tellers in merkblauw, moduleplaten dragen oversized nummers (tot 7rem).
- Verspringing op ≥1000px: hero in 7/4-grid, moduleplaten 02 en 03 schuiven horizontaal in; op mobiel valt alles lineair terug.

## Componentstijl

- **Commandobar:** sticky, marineblauw met 4px oranjegele onderrand en een 6px blauwe scroll-voortgangsbalk (decoratief, `aria-hidden`); wit logo, kapitaal-navigatie (verborgen ≤480px), blauwe bar-CTA. Alle interactieve doelen ≥ 44px.
...
- **Pipeline:** drie omkaderde stations (Artific-station blauw gevuld met oranjegele rand) verticaal met ↓-koppelingen; op ≥1000px horizontaal met →. Koppelingen `aria-hidden`, het geheel heeft een beschrijvend `aria-label`.
