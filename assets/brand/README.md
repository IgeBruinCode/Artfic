...
## Status: `verified` — geverifieerd tegen officiële Artific-PDF-documenten
...
Analoog aan de voorgeschreven demo-CTA-fallback (gebruik de officiële bron en documenteer de afwijking) is daarom geverifieerd tegen de sterkst beschikbare officiële bron: door Artific zélf gepubliceerde PDF-materialen. Die afwijking staat expliciet in het `deviation`-blok van `brand.json`, inclusief de correcties ten opzichte van de eerdere web-afgeleide kandidaten:
...
4. Vul `pdfProvenance` aan met de interne documenten (voeg ze met `available: true` toe aan `referenceDocuments`) en werk het `deviation`-blok bij.
5. Draai `node scripts/validate-content.mjs` tot die slaagt.

## Logo-assets

- `artific-logo-blauw.svg` — woordmerk + AI-beeldmerk, alle paden `#287CEB`; byte-gelijk overgenomen van het door Artific gepubliceerde SVG en per PDF geverifieerd tegen de briefhoofding van het persbericht (kleur pixel-exact). Voor lichte achtergronden.
- `artific-logo-wit.svg` — dezelfde uitvoering, alle paden `#fff`; per PDF geverifieerd tegen covers en footerbanden van whitepaper, brochure en infographic. Voor donkere achtergronden.
