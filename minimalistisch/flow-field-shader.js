(function () {
  ...
  var fragmentSource = `// "Flow field" — made with the 21st.dev Shader Builder
// Packed WebGL1 uniforms (the shader exposes readable u_* aliases as macros):
//   u_colors[8] (first 4 used)
//   Artific Deep Navy, Artific Blue, Artific Light Blue, white
//   u_scene = vec4(canvas width, canvas height, seconds * 0.61, 4.0)
//   u_shape = vec4(1.08, 0.51, 0.50, 0.00)
//   u_surface = vec4(2.40, 0.93, 0.00, 1.00)
//   u_finish = vec4(0.00, 0.00, 0.000, 0.09)
//   u_transform = vec4(3267.0, 0.00, 0.00, 0.0)
...
void main() {
  ...
  function createShader(gl, type, source) {
    ...
    if (!gl.getShaderParameter(shader, gl.COMPILE_STATUS)) {
      console.error("Artific flow shader kon niet compileren:", gl.getShaderInfoLog(shader));
  ...
  function mount(canvas, reducedMotionQuery) {
    ...
    if (!gl.getProgramParameter(program, gl.LINK_STATUS)) {
      console.error("Artific flow shader kon niet linken:", gl.getProgramInfoLog(program));
      canvas.classList.add("is-fallback");
      return function () {};
    }
    ...
    // Artific Deep Navy → Artific Blue → Artific Light Blue → white.
    // Yellow stays a precise UI accent above the shader, rather than a field.
    gl.uniform3fv(uniforms.colors, new Float32Array([
      0.015686, 0.133333, 0.266667,
      0.156863, 0.486275, 0.921569,
      0.898039, 0.929412, 0.972549,
    ...
    gl.uniform4f(uniforms.shape, 1.08, 0.51, 0.50, 0.00);
    gl.uniform4f(uniforms.surface, 2.40, 0.93, 0.00, 1.00);
    gl.uniform4f(uniforms.finish, 0.00, 0.00, 0.000, 0.09);
    gl.uniform4f(uniforms.transform, 3267.0, 0.00, 0.00, 0.0);
    ...
    function draw(now) {
      if (disposed) return;
      resize();
      var seconds = reducedMotionQuery.matches ? 0 : elapsed + (now - runStartedAt) / 1000;
      gl.viewport(0, 0, gl.drawingBufferWidth, gl.drawingBufferHeight);
      gl.uniform4f(uniforms.scene, gl.drawingBufferWidth, gl.drawingBufferHeight, seconds * 0.61, 4.0);
      gl.drawArrays(gl.TRIANGLES, 0, 3);
      if (running && !reducedMotionQuery.matches && !document.hidden && inViewport) {
    ...
    function pause() {
      ...
      raf = 0;
      elapsed += (performance.now() - runStartedAt) / 1000;
      running = false;
    }

    function resume() {
      if (disposed || reducedMotionQuery.matches || document.hidden || !inViewport || running) return;
    ...
    resize();
    if (reducedMotionQuery.matches) draw(runStartedAt);
    else resume();
  ...
  window.ArtificFlowField = { mount: mount };
})();
